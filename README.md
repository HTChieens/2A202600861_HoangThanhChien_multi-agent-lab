...
> Repo hiện có implementation chạy được cho baseline, multi-agent workflow, tracing, search fallback và benchmark report. Học viên có thể thay provider/mock bằng tích hợp production tùy nhu cầu.
...
Lệnh này gọi LLM một lượt và in `ResearchState` kèm trace token/latency.
...
Workflow mặc định chạy `Supervisor -> Researcher -> Analyst -> Writer -> Critic -> done`. Nếu chưa cấu hình `TAVILY_API_KEY`, hệ thống dùng local search fallback và ghi rõ trong `state.errors`.
...
Report được ghi vào `reports/benchmark_report.md`.
...
Các phần runtime chính đã có implementation. Học viên có thể nâng cấp thêm:

1. Thay local search fallback bằng Tavily/Bing/SerpAPI hoặc internal docs.
2. Thay workflow nhẹ hiện tại bằng LangGraph compiled graph nếu muốn demo graph runtime.
3. Gửi trace sang LangSmith, Langfuse hoặc OpenTelemetry.
4. Thêm rubric chấm quality bằng peer review hoặc LLM judge.
5. Thêm citation coverage chi tiết theo từng claim.
